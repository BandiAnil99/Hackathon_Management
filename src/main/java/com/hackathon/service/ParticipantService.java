...
public class ParticipantService {
    ...
    public Participant register(ParticipantRegistrationRequest request, MultipartFile resume, MultipartFile photo) {
        ...
        if (request.candidateType() == CandidateType.INTERNAL && (request.candidateId() == null || request.candidateId().isBlank())) {
            throw new BadRequestException("candidateId must not be null or empty when candidateType is INTERNAL");
        }

        ParticipantRegistrationRequest effectiveRequest = request;
        if (request.candidateType() == CandidateType.EXTERNAL && request.candidateId() != null && request.candidateId().isBlank()) {
            effectiveRequest = new ParticipantRegistrationRequest(
                    request.eventId(),
                    request.name(),
                    request.email(),
        ...
        log.info("Register phase=parallel_tasks_start email={}", effectiveRequest.email());
        ...
        log.info("Register phase=parallel_tasks_done email={} resumeStored={} photoStored={}",
            effectiveRequest.email(), resumeUrl != null, photoUrl != null);

        Participant participant = Participant.builder()
                .eventId(effectiveRequest.eventId())
                .name(effectiveRequest.name())
                .email(effectiveRequest.email())
                .candidateType(effectiveRequest.candidateType())
                .candidateId(effectiveRequest.candidateId())
                .techStack(effectiveRequest.techStack())
                .phone(effectiveRequest.phone())
                .experienceYears(effectiveRequest.experienceYears())
                .resumeUrl(resumeUrl)
                .photoUrl(photoUrl)
            .skills(null)
