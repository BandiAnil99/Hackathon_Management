...
public class FeedbackService {
    ...
    public FeedbackSubmitResponse submit(FeedbackRequest request) {
        ...
        // Recompute feedbackCount and cumulativeAvgScore across all submitted feedbacks
        List<Feedback> allFeedbacks = feedbackRepository.findByParticipantIdOrderBySubmittedAtAsc(participant.getId());
        List<Long> feedbackIds = allFeedbacks.stream().map(Feedback::getId).toList();
        List<FeedbackDetail> allDetails = feedbackDetailRepository.findByFeedbackIdIn(feedbackIds);

        Set<String> ratingFieldNames = feedbackTemplateRepository.findAll().stream()
                .filter(t -> t.getFieldType() == FeedbackFieldType.RATING)
                .map(FeedbackTemplate::getFieldName)
                .collect(Collectors.toSet());

        List<Integer> ratings = allDetails.stream()
                .filter(d -> ratingFieldNames.contains(d.getFieldName()))
                .map(d -> { try { return Integer.parseInt(d.getFieldValue()); } catch (NumberFormatException e) { return null; } })
                .filter(Objects::nonNull)
                .toList();

        double avg = ratings.isEmpty() ? 0.0 : ratings.stream().mapToInt(Integer::intValue).average().orElse(0.0);
        participant.setFeedbackCount(allFeedbacks.size());
        participant.setCumulativeAvgScore(Math.round(avg * 100.0) / 100.0);
